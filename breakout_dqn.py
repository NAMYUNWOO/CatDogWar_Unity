...
class DQNAgent:
    def __init__(self, state_size,action_size,render):
        ...
        if self.load_model:
            self.model.load_weights(os.getcwd()+"\\save_model\\breakout_dqn6_.h5")
        ...
        self.summary_placeholders, self.update_ops, self.summary_op = \
            self.setup_summary()
        self.summary_writer = tf.summary.FileWriter( os.getcwd()+"\\summary\\breakout_dqn", self.sess.graph)
...
if __name__ == "__main__":
    ...
    for e in range(EPISODES):
                ...
                if evt.get(socket) == zmq.POLLIN:
                    ...
                    if done:
                        ...
                        print("episode:", e, "  score:", score, "  memory length:",len(agent.memory), "  epsilon:", round(agent.epsilon,4),
                            "  global_step:", global_step, "  average_q:",agent.avg_q_max / float(step), "  average loss:",agent.avg_loss / float(step))
        ...
        if e % 10 == 0:
            try:
                agent.model.save_weights(os.getcwd()+"\\save_model\\breakout_dqn_.h5")
                pylab.plot(episodes,scores,'b')
                pylab.savefig(os.getcwd()+"\\save_graph\\breakout_dqn_.png")
            except:
                pass
